...
module FinalProject(
    ...
    input changetest, //this is a button to reset 
    ...
    logic [3:0]randNum; //This is the random number that is stored form the register
    logic [3:0]randNumfast; //This is the random number thaat chagnges with each posedge clock
    ...
    logic otherclock; //this is a seperate clock for the random number generator
    logic displayHex = 0; //this is so displayHex is still here, the code breaks without it


    randNumTable RandNumterToDecimal( .RandNumberDecimal(randomNumberDecimal), .clk(clk), .button(changetest), .randNum(randNum));//This takes the amount of times that the user presses the left button and turns it into a 16 bit number for the 7seg display and a 4 bit number for the test 
   ...
   //this block is taking in the RandNum and powering on a FSM
     ...
     //The 15 cases below are the 15 differnt FSM's, they turn on depending on which power is turned on
     FSM_Case_0 FSM_Case_0(.switches(switches),   .power(power0),  .clk(clk), .check(check), .cc(cc0));  //This is FSM 0 leds is for debug use only
   ...
   // hexSevSeg HexSseg(.clk(clk), .switch(randomNumberDecimal), .anode(an), .cathode(ssegs)); //FOR DEBUG USE ONLY this manually overrides the Mux's below
    ...
    Mux #(7) ssegMux(.A(ccsseg), .B(hexsseg), .sel1(displaycc), .sel2(displayHex), .out(ssegs)); //this is a mux to decide which sseg to display //hex and cc are mixed up
    Mux #(3) anMux(.A(ccan), .B(hexan), .sel1(displaycc), .sel2(displayHex), .out(an)); //this is a mux to decide which an to send to the seven segemtn display //hex and cc are mixed up
